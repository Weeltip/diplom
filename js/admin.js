(function () {
  ...
  async function loadCounts() {
    const [{ count: pc }, { count: vc }, { count: rc }, { count: pending }] = await Promise.all([
      ...
      sb.from('vacancies').select('*', { count: 'exact', head: true }).eq('is_published', false)
    ]);
  ...
  function vacancyStatusLabel(isPublished) {
    return isPublished
      ? '<span class="vacancy-status vacancy-status--published">Опубликована</span>'
      : '<span class="vacancy-status vacancy-status--pending">На проверке</span>';
  }
  ...
  async function loadModeration() {
    ...
    data.forEach((v) => {
      ...
      tr.querySelector(`[data-vac-reject="${v.id}"]`)?.addEventListener('click', () => rejectVacancy(v.id, v.title));
    });
  }

  async function approveVacancy(id, title) {
    if (!confirm(`Разрешить публикацию вакансии «${title}»?`)) return;
    const { error } = await sb.from('vacancies').update({ is_published: true }).eq('id', id);
    if (error) {
  ...
  async function rejectVacancy(id, title) {
    if (!confirm(`Отклонить и удалить вакансию «${title}»?`)) return;
    const { error } = await sb.from('vacancies').delete().eq('id', id);
    if (error) {
      showToast(error.message, 'error');
      return;
    }
    showToast('Вакансия отклонена и удалена', 'success');
    void loadModeration();
    void loadCounts();
  }
  ...
  async function loadVacancies(reset) {
    ...
    data.forEach((v) => {
      ...
      tr.innerHTML = `
        ...
        <td>${escapeHtml(v.employer)}</td>
        <td>${formatDate(v.created_at)}</td>
        <td>${vacancyStatusLabel(v.is_published)}</td>
        <td>
